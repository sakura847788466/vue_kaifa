<template>
  <div class="list">
    ...
    <div class="list-item">
      <p class="chunk"><span>*</span><i>dascomId</i>
        <input type="text" placeholder="" v-model="dascomId" maxlength="20">
        <samp class="tip-info"> 打印任务ID <span class="secret-warn-tip"></span></samp>
      </p>
    </div>
...
<script>
  ...
  import {getPrintResult} from '../../api'
  import {validateGetPrintResult} from '../../util/verification'
  ...
  export default {
    ...
    methods:{
      GetDelayPrintTask(){
        const {dsAppid, dsToken,dascomId} = this
        const data = {
          dsAppid,
          dsToken,
          dascomId
        }
        const v = validateGetPrintResult(data)
        if (v) {
        ...
        if (dsAppid && dsToken&&dascomId) {
          this.loading = true
          getPrintResult(dsAppid, dsToken,dascomId).then(res => {
            console.log(res)
            const result = {
              data: res.data,
              status: '成功',
              apiName: '查询打印结果',
              url: 'https://open.dascomyun.com/api/v1.1/getPrintResult'
            }
            this.loading = false
