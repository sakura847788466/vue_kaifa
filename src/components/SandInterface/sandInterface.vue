    ...
    <div class="list">
      <div class="list-item gongsi">
        <p><span class="information">用户信息</span><span class="tag"
                style="font-size: 16px;">（沙箱环境）</span></p>
      ...
      <div class="list-item ds-apid"
        ...
        <div class="list-site">
          <p class="appId"><span onselectstart="return false;"
                  unselectable="on">dsAppid：</span>{{this.userInfo.appid}}
            <!-- ada 3-16 sandData.appid=>this.userInfo.appid -->
            <a style="cursor:pointer; float: right;margin-right: 10px"
               class="copy"
               @click.stop="CopySecret(sandData.appid)">复制</a>
          </p>
        </div>
      </div>
      <div class="list-item miyao"
        ...
        <div class="list-site">
          <p class="secret-key"
             @click="showSecret"><span>密&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;钥：</span>{{InfoClick(sandData.secret)}}
            <a style="cursor:pointer; float: right;margin-right: 10px"
               class="copy"
               @click.stop="CopySecret(sandData.secret)">复制</a>
          </p>
        </div>
      </div>
      ...
      <div class="list-item time">
        <div class="list-site">
          <p class="zcTime"><span onselectstart="return false;"
                  unselectable="on">dsNumber：</span>{{sandData.number}}</p>
        </div>
      </div>
      <div class="list-item time">
        <div class="list-site">
      ...
      <div class="list-item token"
        ...
        <div class="list-site">
          <p class="appId"><span onselectstart="return false;"
                  unselectable="on">访问令牌：</span>{{this.returnToken}}
            <samp>
...
export default {
  ...
  methods: {
    ...
    changeURL () {//修改回调地址
      if (this.returnToken === '') {
        ...
        this.$prompt('请输入回调地址', '提示', {
          ...
          changeUrl(this.sandData.appid, this.returnToken, dataVal)
            .then((res) => {
              console.log(res)
              if (res.code === 0) {
    ...
    getreturnUrl () {//获取回调地址
      getReturnUrl(this.sandData.appid, this.returnToken)
        .then(res => {
    ...
    //获取token
    GetToken () {
      getToken(this.sandData.appid, this.sandData.secret)
        .then(res => {
          this.timeShow = true
    ...
    //获取沙箱环境
    GetSandBox () {
      getSandBox(this.userInfo.appid)
        .then(res => {
          console.log(res);
          this.sandData = res.data
          this.GetToken()
        }).catch((err) => {
          console.log(err);
    ...
    OpenSandBox () {
      if (this.value) {
        openSandBox(this.userInfo.appid)
          .then(res => {
            console.log(res)
            // this.GetSandBox()
          }).catch(err => {
            console.log(err)  //这个函数会走catch 目前存在问题
            this.GetSandBox()
